...
v = df['close_price']
win_max = v.rolling(WIN).max()[WIN:]
win_min = v.rolling(WIN).min()[WIN:]
cv = v[:-WIN]
...
x = list(range(len(df['close_price'])))
...
ax1.scatter(
    ...
    cv[st:ed],
    c=c[st:ed], s=10)
ax1.plot(x[st:ed], win_max[st:ed], 'red')
ax1.plot(x[st:ed], win_min[st:ed], 'green')
ax1.plot(
    x[st:ed],
    cv[st:ed])
ax2 = ax1.twinx()
ax2.plot(x[st:ed], win_max_r[st:ed], 'red')
ax2.plot(x[st:ed], win_min_r[st:ed], 'green')
plt.savefig("res.png")
plt.close()